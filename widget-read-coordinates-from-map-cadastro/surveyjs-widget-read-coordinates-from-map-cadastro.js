...
var widgetMap = {
    ...
    htmlTemplate: "<div><input style='display:none;' /><div id='container-data'></div><button></button><button style='display:none;'></button><div id='map-surveyjs-response' style='display:none;'></div></div>",
    ...
    afterRender: function (question, el) {
        ...
        var divData = el.querySelector("#container-data");
        try{
            divData.innerText = `Objecto: ${JSON.parse(question.value).properties.name}`;
        } catch(e){
        ...
        button.onclick = function () {
            ...
            //register
            document.addEventListener("selectedData",function(data){    
                divData.innerText = `Objecto: ${data.obj.properties.name}`;            
                question.value =  JSON.stringify(data.obj);
            },false);   
            ...
            let mapaClass = new MapasCadastroWidget(
                divMap, 
                ['roadmap','hybrid', 'satellite'], 
                null, 
                {
                    showInfoWindow: true,
                    getCurrentPositionAtStart: true,
                },
                [],
                event
            );          
          
            mapaClass.awaitCurrentPosition(function(){
                mapaClass.getApiCadastroLayers(function(){
                    mapaClass.initMap();
                    var currentCoords = mapaClass.getCurrentCoords();
                    mapaClass.transformJson(currentCoords);
                })
                // var currentCoords = mapaClass.getCurrentCoords();
            ...
            // }
            // markerCreatedByClick = new L.marker(coords).addTo(map);
            ...
            //         }
            //         markerCreatedByClick = new L.marker(e.latlng).addTo(this);
